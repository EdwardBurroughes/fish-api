...
from src.fish.db.models import DBSites, DBSurvey, DBSpecies
...
def validate_offset_and_limit(query_params: QueryParams, count: int) -> PaginationKeys:
    ...
    if skip and limit:
        _validate_paginate_param(int(skip), count, f"skip of {skip} outside of bounds {0} - {count}")
        _validate_paginate_param(int(limit), MAX_LIMIT, f"limit of {limit} outside of bounds {0} - {100}")
...
async def wrap_response_with_pagination_results(request: Request, call_next):
    ...
    if model is not None:
        total_count = get_model_count(model)
        ...
        return await wrap_response_in_meta_data(response, total_count=total_count, next_url=next_url)
    return response
...
def check_for_bad_params(
    ...
    if unexpected_params:
        return JSONResponse(
            status_code=400, content={"details": f"Unexpected query parameters: {unexpected_params}"}
        )
